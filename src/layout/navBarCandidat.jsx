...
const NavBarCandidat=()=>{
    ...
    const logout = () =>{
     ...
     if (result.isConfirmed) {
      candidatService.logout().then((res)=>{
     console.log(res);
   });
  ...
  /*  const [inputText, setInputText] = useState("");
   ...
   const getAll=()=>{
       EntrepriseService.getAllO().then((res)=>{
   ...
   const filteredData = Offres?.filter((el) => {
       if (inputText === '') {
           return el;
       }
       else {
...
useEffect(()=>{
  ...
  style={{marginLeft:'', paddingLeft:''}}/*  onChange={inputHandler}  *//>
  ...
  <div className="container">
    ...
    <div className="collapse navbar-collapse" id="navbarNav">
      ...
      <div className="d-lg-flex align-items-center d-none ms-auto">
        <Link className="custom-btn btn" to="/profilcandidat" style={{margin:'15px'}}>
          Your Profile
        </Link>
